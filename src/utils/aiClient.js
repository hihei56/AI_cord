...
async function getAIResponseOnce(
  ...
  // 人格が設定されているアカウントは、下書きの単語を最低1つは使うことだけ必須にし、
  // 口調・語尾・文法の整え方は人格設定を優先させる(強すぎるとマルコフそのまま、
  // 緩すぎるとマルコフ要素が消える、を何度か行き来した末の中間の強さ)。
  // 人格が無いアカウントは、そもそも「頼る口調」が無いので下書きをベースに
  // 最低限の誤字脱字・助詞の修正だけに留め、言い換えや作文をしないようにする
  const draftSection =
    draft && accountState.persona
      ? `\n【下書き(マルコフ連鎖生成、単語の並びや助詞がおかしいことが多い)】\n${draft}\n上の下書きに出てくる単語を最低1つはそのまま返信に含めること。それ以外の言い回し・口調・語尾は下書きに引きずられず、必ず自分の人格設定に従って自然な一文にすること。下書きに無い新しい話題を付け足すのは禁止。`
      : draft
        ? `\n【下書き(マルコフ連鎖生成)】\n${draft}\n人格設定は無いので、上の下書きをベースに最低限の誤字脱字・助詞の修正だけを行って返信すること。単語の言い換え、文の作り直し、新しい話題や説明の追加はしないこと。`
        : '';
