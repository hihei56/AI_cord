...
async function getAIResponseOnce(
  ...
  // 下書きの拘束を強くしすぎると(過去に「下書きの単語を2つ以上そのまま流用」を
  // 必須にしていた)、人格の口調が埋もれてマルコフ連鎖をほぼそのまま出力しているような
  // 発言になってしまう(実際に発生した問題)。逆に緩めすぎると下書きが無視されて
  // マルコフが全く反映されなくなる(これも過去に発生)。そのため、下書きは
  // 「話題・方向性のヒント」として参考にする程度に留め、口調・語尾・言い回しは
  // 人格設定を優先させる。下書きと人格の口調が衝突したら人格を優先すること
  const draftSection = draft
    ? `\n【下書き(マルコフ連鎖生成、単語の並びや助詞がおかしいことが多い)】\n${draft}\nこの下書きが示す話題や方向性を参考にしつつ、上のあなたの人格・口調で自然な一文を書くこと。下書きの単語をそのまま使う必要は無いが、全く関係ない話題に飛ばないこと。口調や語尾は下書きではなく必ず自分の人格設定に従うこと。`
    : '';
