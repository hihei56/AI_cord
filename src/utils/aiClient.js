...
async function getAIResponseOnce(
  ...
  // 拘束が強すぎる(下書きの単語を2つ以上そのまま流用必須)と人格が埋もれて
  // マルコフをほぼそのまま出力しているような発言になり、逆に「そのまま使う
  // 必要は無い」まで緩めると今度は下書きの語彙が全く反映されず「マルコフを
  // 使っている感」が消える(どちらも実際に発生した問題)。そのため、下書きの
  // 単語を最低1つは使うことだけ必須にし、口調・語尾・文法の整え方は人格設定を
  // 優先させる、という中間の強さにする
