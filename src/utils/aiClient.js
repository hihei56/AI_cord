...
async function getAIResponseOnce(
  ...
  // ただし固定の1文だけを毎回同じ言い回しで渡すと、どのアカウントが中心/聞き役に
  // なっても同じような発言パターンに収束し、掛け合いが規則的で機械的に見える原因に
  // なっていた。複数の言い回しをランダムに選ぶことで単調さを崩し、かつ「自分の
  // キャラクターに合うやり方で」を必ず添えて、人格ごとの個性が役割に埋もれないようにする
  const ROLE_INSTRUCTIONS = {
    center: [
      '自分の話や具体的なエピソードを振ったり、相手に質問したりして会話を引っ張ること。ただし1つの話題に固執しすぎず、自然に広げること。',
      'あなたから話しかけるような流れにすること。ただし話題を丁寧に説明しすぎず、自分のキャラクターらしいテンションで振ること。',
      '相手が答えやすい具体的な話や小ネタを振って会話の主導権を握ること。毎回質問形にする必要はない。'
    ],
    reactor: [
      '相手の話に短く相槌を打ったり感想を返したりすることを中心にし、自分から新しい話題を広げすぎないこと。',
      '基本は聞き役に回り、相手の発言を受けて短く反応すること。たまに自分の話を混ぜても構わない。',
      '相手の話にちゃんと反応しつつ、自分から強く話題を変えないこと。素っ気ない一言だけの反応でもよい。'
    ]
  };
  const roleInstructionPool = ROLE_INSTRUCTIONS[role];
  const roleSection = roleInstructionPool
    ? `\n【この会話でのあなたの役割】${roleInstructionPool[Math.floor(Math.random() * roleInstructionPool.length)]}人格設定やキャラクターは変えず、あくまで自分らしいやり方でこの役割をこなすこと。`
    : '';
