...
function loadMealpostAccounts() {
  ...
  if (firstToken) {
    accounts.push({
      ...
      mealChannelId: process.env.MEALPOST_CHANNEL_ID,
      mealFolder: process.env.MEALPOST_IMAGE_FOLDER,
  ...
  while (process.env[`MEALPOST_DISCORD_TOKEN_${i}`]) {
    accounts.push({
      ...
      mealChannelId: process.env[`MEALPOST_CHANNEL_ID_${i}`],
      mealFolder: process.env[`MEALPOST_IMAGE_FOLDER_${i}`],
...
const clients = accounts.map((account) => {
  ...
  client.accountState = {
    ...
    // ご飯画像の投稿先/画像フォルダ。未設定ならconfig/settings.jsonのmealPostsの値を使う
...
for (const client of clients) {
  client.once('ready', () => {
    ...
    logger.log('READY', `[${client.accountState.id}] 参加サーバー${guilds.length}件: ${guilds.join(', ')}`);
