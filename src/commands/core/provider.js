...
module.exports = {
  ...
  aliases: ['ai'],
  description:
    '会話生成に使うAIプロバイダ(groq/gemini/cerebras/nvidia)を実行中に切り替える。!provider [groq|gemini|cerebras|nvidia] (引数省略で現在の状態を表示)。.envの書き換え・再起動不要、全アカウント共通',
    ...
    if (!target) {
      ...
      return msg.channel.send(
        `現在のAIプロバイダ: **${aiProvider.getProvider()}**\n` +
          `切り替え可能(APIキー設定済み): ${available.length ? available.join(', ') : '(GROQ_API_KEY/GEMINI_API_KEY/CEREBRAS_API_KEY/NVIDIA_API_KEYが.envに未設定)'}\n` +
          `使い方: !provider groq|gemini|cerebras|nvidia`
      );
    }
